...
export default function ChatBot() {
	...
	const sendMsg = (msg: string) => {
		...
		if (msg.trim()) {
			setMessages([...messages, { text: msg, isBot: false, idx: messages.length }]);
			input.value = ""
			autoResizeInput()
			setDisabled(true)
			ajax.ajax({
	...
	return (
		<div class="w-full h-full flex flex-col justify-between">
			<div class="w-full self-center text-center h-auto text-3xl bg-slate-100"> Ascend LLM</div>
				...
				<For each={messages}>
					{(item, index) => (
						<div class={(item.isBot ? "flex-row " : "flex-row-reverse ") + 'flex my-4'}>
							...
							<div class={(item.isBot ? "bg-yellow-100" : "bg-blue-100") + " p-4 mx-8 rounded-lg max-w-[900px]"}><MarkdownRoot content={item.text} setProcess={setProcess}></MarkdownRoot></div>
						</div>
			...
			<div class="flex w-full items-center">
				<div class="overflow-scroll [&amp;:has(textarea:focus)]:border-token-border-xheavy [&amp;:has(textarea:focus)]:shadow-[0_2px_6px_rgba(0,0,0,.05)] flex flex-col w-full flex-grow relative border dark:text-white rounded-2xl bg-token-main-surface-primary border-token-border-medium justify-center">
					<button class="absolute left-2 rounded-lg border border-black bg-black p-0.5 text-white transition-colors enabled:bg-black disabled:text-gray-400 disabled:opacity-10 dark:border-white dark:bg-white dark:hover:bg-white md:left-3" data-testid="clear-button" onclick={clearHistory} disabled={getDisabled()} >
					...
					<textarea id="prompt-textarea" tabindex="0" data-id="433be95d-99e1-4924-ba99-b4c68e55cea1" dir="auto" rows="1" placeholder="给Ascend LLM发送消息" class="m-0 w-full resize-none border-0 bg-transparent focus:ring-0 focus-visible:ring-0 dark:bg-transparent pr-10 py-3.5 md:pr-12 placeholder-black/50 dark:placeholder-white/50 pl-10 md:pl-14 outline-none h-auto max-h-none" ref={input}></textarea>
					<button class="absolute  right-2 rounded-lg border border-black bg-black p-0.5 text-white transition-colors enabled:bg-black disabled:text-gray-400 disabled:opacity-10 dark:border-white dark:bg-white dark:hover:bg-white md:right-3" data-testid="send-button" onclick={handleSendMessage} disabled={getDisabled()} >
